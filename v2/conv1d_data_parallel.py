...
class ConvNetOneDataParallel(Transform):
    ...
    def forward(self, inputs, y_labels):
        ...
        (losses, preds, softmax, new_labels) = self.loss.forward(linear_out, y_labels, get_predictions=True)
        return (losses, preds, relu_x_mult, linear_input, softmax, new_labels, maxpool_input_shape, maxpool_grad_mask)

    def backward(self, inputs, relu_x_mult, linear_input, softmax, new_labels, maxpool_input_shape, maxpool_grad_mask):
        loss = self.loss.backward(softmax, new_labels)
    ...
    def update(self, learning_rate, momentum_coeff, linear_out, conv_out):
        self.linear.update(learning_rate, momentum_coeff, linear_out[0], linear_out[1])
        self.conv.update(learning_rate, momentum_coeff, conv_out[0], conv_out[1])