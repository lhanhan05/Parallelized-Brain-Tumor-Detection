...
Linear::Linear(int in_features, int out_features) {
    ...
    // this->debug = debug; // Uncomment if debugging functionality is added
}

// Forward pass function
std::vector<std::vector<double>> Linear::forward(const std::vector<std::vector<double>>& A) {
    Atemp = A;
...
std::vector<std::vector<double>> Linear::backward(const std::vector<std::vector<double>>& dLdZ) {
    int N = dLdZ.size();
    int C1 = dLdZ[0].size();
